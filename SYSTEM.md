...
- **OPD Notification Repeat (60s while Unaccepted)**: `opdNotifiedVisitIds` in [src/main.js](src/main.js) changed from `Set<visitId>` to `Map<visitId, lastNotifiedAtMs>`. New constant `OPD_NOTIFICATION_REPEAT_MS = 60_000`. `handleOpdQueueNotification` now gates on `(now - lastNotifiedAt) < REPEAT_MS` instead of `.has()`, so any visit still in `Status='Waiting OPD'` (doctor hasn't pressed ຮັບ) re-fires the toast + sound + desktop notification every 60s. `pollOpdQueueNotifications` (15s poll) prunes Map entries whose Visit_ID is no longer in the waiting set, so an accepted-then-re-added visit notifies immediately. `seedOpdNotifiedVisits` seeds existing waiters with `Date.now()` to avoid a flood on login — first re-notification fires ~60s later. Verified via fake-clock harness: 0s fires, 20/59s gated, 61s re-fires, 125s re-fires again, post-accept silent.
