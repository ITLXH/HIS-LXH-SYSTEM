...
- **Patient Modal UX Overhaul (2026-06-24 — LOCAL TEST PENDING)**: Five changes to `patientModal` ([public/partials/modals/patient-modal.html](public/partials/modals/patient-modal.html) + [src/main.js](src/main.js)).
  ...
  5. **Relationship → master-data dropdown**: `#p_emer_relation` is now `<select class="dyn-EmergencyRelation">`. New master category `EmergencyRelation` wired into `_masterDataFallback`, `seedMasterDefaults` (default values: ພໍ່/ແມ່/ຜົວ/ເມຍ/ລູກ/ພີ່/ນ້ອງ/ຍາດພີ່ນ້ອງ/ໝູ່/ອື່ນໆ), `loadMasterDataGlobalCallback`'s populate-list, `resetMasterDefaults` whitelist, and added to `masterCategoryGroups[patient]` so it shows up in **ຕັ້ງຄ່າ → ຄົນເຈັບ → ຄວາມສຳພັນ (ຕິດຕໍ່ສຸກເສີນ)** with full CRUD. Seed flag bumped from `his_master_seeded_v1` → `his_master_seeded_v2` so existing installs re-check and insert the new category on next login.
