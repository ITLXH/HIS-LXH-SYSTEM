...
- **Patient Modal UX Overhaul (2026-06-24 — LOCAL TEST PENDING)**: Five changes to `patientModal` ([public/partials/modals/patient-modal.html](public/partials/modals/patient-modal.html) + [src/main.js](src/main.js)).
  ...
  2. **Discount field scrollable + tooltip**: `#p_discount_show` keeps the readonly look but releases `readonly` on focus (so the text cursor can arrow-scroll long values) and re-applies it on blur; keystroke handler blocks any key except arrow/home/end/tab so the value can't be edited. `fetchOrg` also sets the `title` attribute to the full discount text so hover shows the whole line.
