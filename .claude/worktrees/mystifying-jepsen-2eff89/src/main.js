...
window.loadUsers = async function () {
  ...
  try {
    const { data: u, error } = await supabaseClient.from('HIS_One_Users').select('*');
...
window.deleteUserRow = async function (id) {
  ...
  if (r.isConfirmed) {
    const { error } = await supabaseClient.from('HIS_One_Users').delete().eq('ID', id);
    if (error) {
      Swal.fire('Error', error.message, 'error');
    } else {
      window.loadUsers();
...
window.submitUserForm = async function (e) {
  ...
  if (isEdit) {
    let { error } = await supabaseClient.from('HIS_One_Users').update(row).eq('ID', $('#u_id').val());
    ...
    let { error } = await supabaseClient.from('HIS_One_Users').insert(row);
    if (error) { Swal.fire('Error', error.message, 'error'); return; }
...
window.bulkDelete = async function (type) {
  ...
  const config = {
    ...
    'users': { table: 'HIS_One_Users', col: 'ID', reload: window.loadUsers },
